...
std::string MasterMetricManager::get_summary_string(
    ...
    // Uses PreciseLatencyTracker for exact avg/p50/p95/p99/min/max values.
    ss << "\n=== Eviction & Cache Lookup Latency (microseconds) ===\n";
    ss << eviction_lat_precise_.format("Eviction(total)") << "\n";
    ss << mem_eviction_lat_precise_.format("Eviction(mem)") << "\n";
    ss << nof_eviction_lat_precise_.format("Eviction(nof)") << "\n";
    ss << cache_lookup_lat_precise_.format("CacheLookup(total)") << "\n";
    ss << cache_lookup_hit_lat_precise_.format("CacheLookup(hit)") << "\n";
    ss << cache_lookup_miss_lat_precise_.format("CacheLookup(miss)");
